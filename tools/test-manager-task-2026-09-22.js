...
/* 🔴 22.9 — **ההחלטה התהפכה, ביודעין** (הכרעת יועד: התיאור "צריך להיות
   מוצג לכולם"). הבדיקה הזאת שמרה ש"אין תיאור" עד שיוחלט אחרת; עכשיו היא
   שומרת את ההפך: השדה קיים בכללים, נכתב, נאסף בטופס ומוצג בכרטיס. */
section('3. תיאור — קיים בכללים, במסמך ובטופס (22.9)');
const gtFields = (RULES.match(/function gtFields\(\)[\s\S]*?\}/) || [''])[0];
ok('gtFields מכיל desc', /['"]desc['"]/.test(gtFields));
const docLiteral = create.slice(create.indexOf('var doc = {'), create.indexOf('CBA.fb.createDoc'));
ok('המסמך שנכתב כולל desc', /(^|[^\w])desc\s*:/.test(docLiteral));
ok('הטופס אוסף תיאור', /gf-desc/.test(GF));
