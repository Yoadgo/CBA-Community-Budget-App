...
ok('🔴🔴 והסטטוס נגזר בנקודה אחת', /function clubStatusOf_\(ev\)/.test(GS) &&
   (GS.match(/clubStatusOf_\(/g) || []).length === 5,
...
/* 🔴🔴 ורשימת הניהול עברה אף היא לנקודה האחת (16.9) — עד אז נשארה בה
   הגדרה מקבילה שאיש לא ראה. */
ok('🔴🔴 וגם רשימת הניהול גוזרת ממנה', /note: ev\.getTag\('note'\) \|\| '',[\s\S]{0,260}status: clubStatusOf_\(ev\)/.test(GS));
