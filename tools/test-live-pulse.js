...
ok('🔴 וכל טריגר מפורש כופה מחזור',
   (APP.match(/doPoll\(true\)/g) || []).length === 6,
...
/* 🔴🔴 הבאג שנתפס בהדלקה הראשונה (15.9): קריאה אחת בטעינת
   המודול מתחרה בשחזור הזהות — `authReady` מחזירה `null` אחרי
   4 שניות, והמאזין לא עולה אף פעם. **בלי שום שגיאה** — הסקר
   ממשיך לעבוד והצעד פשוט לא קורה. */
ok('🔴 ההדלקה חוזרת גם אחרי התחברות וגם אחרי הציור הראשון',
   (APP.match(/window\.CBA\.startPulse\(\);/g) || []).length === 2,
   String((APP.match(/window\.CBA\.startPulse\(\);/g) || []).length));
ok('⚠️ והיא אידמפוטנטית — מאזין שכבר חי אינו נפתח שוב',
   /function startPulse\(\) \{\s*\n\s*if \(pulseStop \|\|/.test(APP));
