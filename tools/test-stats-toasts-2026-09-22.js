...
section('2. ממצא 25 — העברית (המסך הוחלף ב-22.9)');
/* 🔴 22.9 — מסך הנתונים נבנה מחדש לפי אפיון גרסה 3 (ר' test-garden-stats-2026-09-22.js).
   המשפט "שבוע אחד שהסתיים" לא קיים יותר; מה שנשאר לשמור הוא שבורר
   התקופה **משנה את כל מספרי התקופה** ולא מצויר סתם. */
ok('🔴 בורר התקופה מחשב מחדש ומצייר', /weeks = w; recompute\(\); draw\(\);/.test(ST), 'חסר');
ok('ומקבל 4/8/12 בלבד', /\[4, 8, 12\]\.map/.test(ST));
...
ok('🔴 בורר התקופה משנה את מספרי התקופה — המנוע מקבל אותו', /weeks: weeks, categories: raw\.categories/.test(ST), 'חסר');
