...
ok('שמונה-עשר דגלים ברשימה (24.9: +appReportsFromFirestore)', sandbox.FLAG_KEYS.length === 18, JSON.stringify(sandbox.FLAG_KEYS));
/* 🔴 גל 3 (18.9) — הדגל שמחזיר את כל פעולות המנהל ל-Apps Script
   בלי דיפלוי. זה התנאי שאפשר להעביר את כולן בסבב אחד. */
ok('ו-gardenWritesFromBrowser ביניהם',
   sandbox.FLAG_KEYS.indexOf('gardenWritesFromBrowser') !== -1, JSON.stringify(sandbox.FLAG_KEYS));
...
/* 🔴 צעד 10 — הפעימה החיה. כיבוי מחזיר את הסקר המלא. */
ok('pulseToFirestore — המתג של צעד 10',
   sandbox.FLAG_KEYS.indexOf('pulseToFirestore') !== -1);
...
ok('flagsGet מחזירה מצב ורשימה', g.ok === true && g.keys.length === 18 && g.flags.servicesFromFirestore === true,
   JSON.stringify(g));
