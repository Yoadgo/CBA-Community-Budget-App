...
ok('שישה דגלים ברשימה', sandbox.FLAG_KEYS.length === 6, JSON.stringify(sandbox.FLAG_KEYS));
...
ok('flagsGet מחזירה מצב ורשימה', g.ok === true && g.keys.length === 6 && g.flags.servicesFromFirestore === true,
   JSON.stringify(g));
