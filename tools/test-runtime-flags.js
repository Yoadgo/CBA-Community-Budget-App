...
ok('שלושה-עשר דגלים ברשימה', sandbox.FLAG_KEYS.length === 13, JSON.stringify(sandbox.FLAG_KEYS));
...
ok('flagsGet מחזירה מצב ורשימה', g.ok === true && g.keys.length === 13 && g.flags.servicesFromFirestore === true,
   JSON.stringify(g));
