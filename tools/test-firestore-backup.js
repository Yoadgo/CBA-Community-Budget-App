...
ok('ארבעת האוספים שיש בהם מידע', sandbox.BK_COLLECTIONS.length === 4,
   String(sandbox.BK_COLLECTIONS.length));
['gardenPlan', 'gardenMeta', 'services', 'appConfig'].forEach(c => {
...
section('6. הגיבוי המלא — ריצה מקצה לקצה');
{
  ...
  ok('ארבעה טאבים', r.tabs.length === 4, String(r.tabs.length));
...
}
{
  ...
  const r = sandbox.fsBackupAll_(ss);
  ok('🔴 כישלון באוסף אחד לא מפיל את השאר', r.tabs.length === 3, String(r.tabs.length));
  ...
  ok('read סופר רק מה שנקרא בפועל', r.read === 3, String(r.read));
}
