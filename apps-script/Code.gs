...
/* ============================================================================
 *  🔴 ממצא 30 — למה התוקף ירד מ-14 יום ל-24 שעות   (2026-09-22)
 * ----------------------------------------------------------------------------
 *  **הסיכון:** אסימון המושב נוסע במחרוזת השאילתה של כל קריאת `GET`
 *  (`?session=<base64>.<חתימה>&action=…`). המשמעות היא שהוא נכתב **ללוג
 *  ההפעלות של Apps Script**, נשמר בהיסטוריית הדפדפן, ועלול לדלוף בכותרת
 *  `Referer`. החתימה עצמה תקינה — ארבע התקפות נבדקו ונדחו — אבל מי
 *  שמשיג אסימון חי מלוג ישן יכול להשתמש בו **עד שיפוג**.
 *
 *  🔑 **והנה מה שהכריע: שתי תקרות הזמן לא דיברו זו עם זו.** הלקוח מוחק
 *  את המושב מ-`localStorage` אחרי **12 שעות** (`SESSION_TTL` ב-app.js),
 *  ומאותו רגע המשתמש מתחבר מחדש דרך גוגל וממילא מקבל אסימון חדש.
 *  כלומר 14 הימים לא שירתו **אף משתמש** — הם רק הרחיבו את החלון שבו
 *  אסימון שדלף מלוג נשאר שמיש.
 *
 *  24 שעות = פי שניים מתקרת הלקוח, מרווח נוח לסטיית שעון ולמקרי קצה,
 *  **ואפס עלות למשתמש** — הוא מתחבר מחדש כל 12 שעות ממילא.
 *
 *  ⏭ **התיקון המלא הוא להוציא את האסימון מהכתובת**, כלומר להעביר גם את
 *     הקריאות ל-`POST` עם המושב בגוף — המנגנון כבר קיים ומוכח
 *     (`postRead` ב-sheets.js, בקשה "פשוטה" בלי preflight). זה לא נעשה
 *     כאן כי זה נוגע ב-26 פעולות `GET`, ב-`GET_ACTION_PERMS`, ובאיחוד
 *     הבקשות הכפולות (`inFlightGets`) — כלומר במסלול הקריאה הרגיש ביותר
 *     לביצועים, שלקח שלושה גלים לכוונן. גל בפני עצמו.
 * ========================================================================== */
...
function doGet(e) {
  try {
    ...
    /* ====================================================================
     *  דיאטת המטען  (2026-09-14, שלב ב3)
     * --------------------------------------------------------------------
     *  📊 נמדד על הייצור: המטען היה 99.1KB, מתוכם **97%** נתוני תקציב
     *  ו-**71%** תנועות של *שנה שעברה* — מידע שאף אחד לא מסתכל עליו ברגע
     *  הנחיתה, כשהמסך שאליו נוחתים הוא בכלל אזור התושב.
     *
     *  מעכשיו **מי שיש לו הרשאת תקציב** מקבל בפתיחה רק את השנה הנוכחית;
     *  שנים קודמות נמשכות לפי דרישה דרך `budgetYear` (ר' handleBudgetYear_).
     *
     *  🔴 **ומי שאין לו הרשאת תקציב ממשיך לקבל את כל השנים.** זה לא פשרה
     *     אלא תנאי: DATA_MIN מחזיר לתושב רק את שורות *משפחתו* (זעיר), והן
     *     מזינות את `myRequestsAllYears` — היסטוריית ההחזרים חוצת-השנים
     *     שנבנתה במפורש ב-7.9. דיאטה גורפת הייתה מוחקת לתושב את כל
     *     ההיסטוריה שלו **בלי שום שגיאה**, רק מספרים חסרים.
     *
     *  ⚠️ `out.years` ממשיך להכיל את **כל** השנים — הבורר חייב להציג אותן
     *     כדי שיהיה למה לעבור. מה שמצטמצם הוא `out.data` בלבד.
     * ==================================================================== */
    var currentY = String(out.currentYear || '').trim();
    /* 🔴 **הדיאטה מופעלת רק כשהלקוח מבקש אותה במפורש (`slim=1`).**
     *  בלי הדגל הזה היה חלון סיכון אמיתי: לקוח ישן שיושב במטמון (PWA פתוח
     *  שלא רוענן מאז אתמול) **אינו יודע למשוך שנה חסרה**, והיה מקבל מטען
     *  בלי תשפ"ו ומציג אותה כתקציב ריק — בלי שגיאה ובלי דרך לתקן.
     *  ⚠️ השרת לא מנחש יכולות של לקוח. הלקוח מצהיר עליהן.
     *  הדגל נוסף ב-sheets.js:fetchAndApply. כשכל הלקוחות יתעדכנו אפשר
     *  יהיה להפוך אותו לברירת מחדל — אבל **לא לפני זה**. */
    var slimRaw = String((e && e.parameter && e.parameter.slim) || '');
    var clientSlim = slimRaw === '1' || slimRaw === '2';
    var slimYears = seesBudget && clientSlim && currentY && years.indexOf(currentY) !== -1;

    /* ========================================================================
     *  🔴 **ההיפוך** (צעד 09ב-5ג, 2026-09-15)
     * ------------------------------------------------------------------------
     *  `slim=2` = "אני יודע למשוך גם את **התנועות** בעצמי". ברגע שהדגל
     *  דלוק, המטען מפסיק לשלוח אותן והלקוח קורא אותן מ-Firestore.
     *
     *  🔴 **התקציב עצמו ממשיך להישלח** — הוא עובר רק בצעד 09ג. כלומר
     *  השנה הנוכחית מגיעה מעורבת: תוכנית מהגיליון, תנועות מ-Firestore.
     *  זה מכוון, וזה מה שמאפשר להפוך תחום אחד בלי לגרור את השני.
     *
     *  ⚠️ **השרת מכריע, לא הלקוח.** הלקוח מצהיר על יכולת; הדגל החי
     *     קובע. כך מתג החירום מכבה את כולם בבת אחת, ולקוח ישן
     *     (`slim=1`) ממשיך לקבל תנועות ולעבוד כרגיל.
     ...
     * ------------------------------------------------------------------------
     *  עד כאן הלקוח הכריע על **כתיבה** לפי מפת הדגלים
     *  שנקראת פעם אחת ב-`ensureDb`, בטעינת העמוד. נתפס חי
     *  (15.9): אחרי `flagSet` השרת מתעדכן תוך דקה — אבל
     *  **לשונית שכבר פתוחה אף פעם לא**, עד רענון עמוד.
     *  כלומר הקריאה והכתיבה מתהפכות בזמנים שונים — וזה
     *  הפיצול שכל הצעד הזה בא למנוע: תנועה שנכתבת
     *  למקום אחד בזמן שקוראים מהמקום השני פשוט **נעלמת**.
     *
     *  לכן המטען נושא את הדגל החי לכל משתמש, והלקוח
     *  מעדכן את שער הכתיבה ממנו בכל משיכה. מתג החירום
     *  תופס מעכשיו לכולם תוך מחזור סקר אחד, בלי רענון עמוד.
     *
     *  ⚠️ **בלי `seesBudget` בכוונה.** זו עובדה על המערכת, לא
     *     הרשאה — ותושב שמגיש בקשת החזר חייב אותה בדיוק
     *     כמו הגזבר. `txFromFirestore` הוא אחר: הוא אומר "רוקנתי
     *     לך את התנועות", וזה אכן תלוי בהרשאה.
     * ====================================================================== */
    out.txFsOn = txJobsUseFirestore_();
    ...
    years.forEach(function (y) {
      ...
      out.data[y] = {
        budget: yd.budget,
        income: yd.income,
        /* 🔴 ריק **במכוון** כשההיפוך דלוק — הלקוח ממלא מ-Firestore.
           ר' ההערה הארוכה למעלה.
           ⚠️ **`y === currentY` הוא רשת ביטחון, לא קישוט.** במסלול
              הרגיל בכלל נשלחת רק השנה הנוכחית, אבל אם `slimYears`
...
var DEFAULT_EMAIL_SETTINGS = [
  ...
  ['GARDEN_REPORT_RECEIVED', "קיבלנו את דיווח הגינון שלך: {{כותרת}} (מס' {{מזהה}})",
    "שלום {{שם}},\n\nקיבלנו את הדיווח שלך: {{כותרת}} ({{קטגוריה}}, {{מיקום}}) (מס' {{מזהה}}). הוא הועבר לצוות הגינון, ואפשר לעקוב אחרי הסטטוס באפליקציה.\n\nבברכה,\nועד הקהילה",
  ...
  /* ⚠️⚠️ **שתי התבניות הבאות נוספו ב-22.9 (ממצא 32), והן נכתבות
     לגיליון מעצמן** — `ensureEmailSettingsSheet_` מוסיפה כל מפתח
     שחסר בכל קריאה. אין שום עבודה ידנית בטאב "הגדרות מיילים".
     🔴 אבל **שם שאינו ב-`GARDEN_TASK_TEMPLATES` וב-`gtNotifyOk`
        בכללים לא יישלח לעולם** — שלוש רשימות, ושלושתן חייבות
        להכיל את השם. `tools/test-finding32-timeline.js` שומר על זה. */

  /* שינוי מועד. עד 22.9 שיבוץ מחדש ודחייה לשבוע הבא היו שקטים
     לחלוטין מבחינת התושב — הוא ראה "מתוכנן" ולא ידע שהמועד זז. */
  ['GARDEN_RESCHEDULED', "עדכון מועד לדיווח שלך (מס' {{מזהה}})",
    "שלום {{שם}},\n\nמועד הטיפול בדיווח שלך על {{קטגוריה}} ב{{מיקום}} (מס' {{מזהה}}) " +
    "עודכן לשבוע של {{שבוע}}.\n\n{{מה נעשה}}אפשר לעקוב אחרי הדיווח באפליקציה בכל רגע.\n\n" +
    "בברכה,\nועד הקהילה",
    'נשלח לתושב כשצוות הגינון משבץ את הדיווח לשבוע אחר, או דוחה אותו לשבוע הבא. ' +
    '{{שבוע}} הוא המועד החדש, {{מה נעשה}} הוא הסבר חופשי אם הצוות כתב אחד',
    PERM_GARDEN, 'כן'],

  /* עדכון סטטוס באמצע הדרך. [stated] יועד (21.9): "הוספת הערת ביצוע או
     דיווח סטטוס הם היום אותו דבר — בפועל הם צריכים להיות מצטברים ולא
     מתחלפים." היומן מצטבר, וזה המייל שמלווה אותו. */
  ['GARDEN_STATUS_NOTE', "עדכון על הדיווח שלך (מס' {{מזהה}})",
    "שלום {{שם}},\n\nיש עדכון מצוות הגינון על הדיווח שלך על {{קטגוריה}} ב{{מיקום}} " +
    "(מס' {{מזהה}}):\n\n{{עדכון}}\n\nאת כל מה שקרה עם הדיווח אפשר לראות באפליקציה.\n\n" +
    "בברכה,\nועד הקהילה",
    'נשלח לתושב כשהצוות כותב הערת ביצוע או עדכון סטטוס על דיווח שלו. ' +
    '{{עדכון}} הוא הטקסט שנכתב',
    PERM_GARDEN, 'כן'],
  /* ⚠️ 2026-09-09 — הנוסח שונה. עד היום היה כתוב "הושלם **ואושר על ידי
     הוועד**", ומשבוטל שלב האישור זה הפך לאמירה לא נכונה. במקום האישור נכנס
     {{מה נעשה}} — הערת הביצוע של הצוות, שהיא חובה בדיווח תושב. */
  ['GARDEN_COMPLETED', "הטיפול בדיווח שלך הושלם (מס' {{מזהה}})",
    "שלום {{שם}},\n\nהטיפול בדיווח שלך על {{קטגוריה}} ב{{מיקום}} הושלם.\n\n{{מה נעשה}}{{איחוד}}אם משהו לא נראה לך תקין — אפשר להשיב לנו באפליקציה בתוך שבוע.\n\nתודה שדיווחת,\nועד הקהילה",
    'נשלח לתושב כשצוות הגינון מסמן "בוצע". {{מה נעשה}} מתמלא בהערת הביצוע, {{איחוד}} במשפט על האיחוד אם היה', PERM_GARDEN, 'כן'],
...
function dailyEmailJobs_() {
  ...
  /* הגיבוי המלא (2026-09-15, צעד 07ד).
     ⚠️ **נתלה על הטריגר היומי הקיים ולא על טריגר נפרד** — אותו
        שיקול שנרשם ליד yearRolloverJob_: טריגר נוסף הוא עוד מנגנון
        שצריך להתקין ולזכור. האפיון אמר "בלילה"; ב-8:00 ההפרש חסר
...
/* ============================================================================
 *  העבודה השעתית   (צעד 07ד, 2026-09-15)
 * ----------------------------------------------------------------------------
 *  האפיון אמר "כל 30 דקות". **יועד הכריע שעה** (15.9) — מחצית
 *  את מספר ההפעלות (24 ביום במקום 48) בלי לשנות שום דבר מהותי:
 *  הפיגור המקסימלי של הגיבוי עולה מ-30 דקות לשעה.
 *
 *  ⚠️ **מה היא עושה היום:** גיבוי מצטבר בלבד. סחיטת תיבת הדואר
 *     (מסמכים עם `mailPending`) **טרם קיימת**, כי אין עדיין כתיבה
 *     מהדפדפן. היא תיכנס לכאן באותו דפוס, **באותה ריצה**, כשתגיע.
 *     לא להתקין טריגר נוסף עבורה.
 *  ⚠️ **השם בלי קו תחתון במכוון** — כדי שהיא תופיע ברשימת
 *     הפונקציות במסך "מפעילים" ואפשר יהיה להתקין אותה מהממשק
 *     במקום להריץ פונקציה מבורר הפונקציות (ר' מלכודת seedGardenPlan).
...
/* התקנה מהעורך (חלופה למסך "מפעילים").
   ⚠️ מוחקת קודם טריגרים קיימים לאותה פונקציה — אחרת הרצה שנייה
      מתקינה שני טריגרים שרצים במקביל. */
function installHourlyTrigger() {
  ScriptApp.getProjectTriggers().forEach(function (t) {
    if (t.getHandlerFunction() === 'hourlyJobs') ScriptApp.deleteTrigger(t);
  });
  ScriptApp.newTrigger('hourlyJobs').timeBased().everyHours(1).create();
  Logger.log('✓ טריגר שעתי הותקן — גיבוי מצטבר כל שעה');
}
...
var FLAG_KEYS = ['gardenPlanFromFirestore', 'servicesFromFirestore', 'budgetYearFromFirestore',
  ...
  /* 🔴 **המתג של מיגרציית הגינון** (2026-09-16) — "הדיווחים שלי"
     ...
     ⚠️ **הכתיבה בשרת אינה תלויה בדגל** — המסמכים מתעדכנים גם
        כשהוא כבוי, כך שהדלקה מוצאת נתונים טריים ולא אוסף ריק.
     ⚠️ ברירת המחדל בקוד הלקוח היא `true`, כי `fsFirstRead` מקצרת
...
function btxRow_(row) {
  ...
  /* 🔴🔴 **החריג היחיד לכלל "שם אדם לא עובר ל-Firestore"**
     (הכרעת יועד, 15.9.2026). בדרך כלל שם הרוכש אינו נכתב,
     והלקוח מרכיב אותו ממזהה המשפחה. אבל לשורה **בלי מזהה
     משפחה** אין ממה להרכיב, והתא היה נשאר ריק במסך הגזבר.
     נמדד: 38 שורות כאלה בתשפ"ו, כולן היסטוריות.
     ⚠️ **החשיפה זהה למה שקיים היום:** השורות האלה נכנסות לדלי
     `__none__`, ו-`canSeeFamilyTx` חוסם אותו לבעלי הרשאת תקציב בלבד —
     בדיוק אותו קהל שרואה אותן היום דרך Apps Script.
     🔴 **שורה עם מזהה משפחה לעולם לא תשא שם** — אל תהפוך את זה
     ל"תמיד מעבירים רוכש", זו הרחבה של החריג על כל הקהילה. */
  if (!String(row['מזהה משפחה'] == null ? '' : row['מזהה משפחה']).trim()) {
    var buyer = String(row['רוכש'] == null ? '' : row['רוכש']).trim();
    if (buyer) out['רוכש'] = buyer;
  }
  return out;
}
...
function handleBudgetTxApply_(p) {
  try {
     ...
     * ------------------------------------------------------------------------
     *  הנתיב הזה נקרא **רק מהדפדפן**, אחרי שכתב תנועה
     *  ישירות ל-Firestore (הטריגר השעתי קורא ל-`budgetTxApplyPending_`
     *  ישירות ולא עובר כאן). כתיבה כזאת אינה נוגעת בשרת
     *  בכלל, ולכן המונה לא זז מעצמו — ובלי שהוא יזוז,
     *  הדפדפן של **כל אדם אחר** לא ידע שיש מה למשוך.
     *
     *  ⚠️ `budgetTxApplyPending_` מרימה רק כשהיא נגעה בגיליון,
     *     וזה נכון לה — אבל הוספה, מחיקה ועריכת פרטים אינן
     *     "ממתינות" לכלום ולכן לא נוגעות בגיליון כאן.
     *  ⚠️ המחיר: הרמה מיותרת = משיכה מלאה אחת מיותרת.
     *     המחיר ההפוך הוא נתון שלא מגיע — והוא גרוע בהרבה.
     * ====================================================================== */
 ...
 * ========================================================================== */
/* 🔴 **הדגל נקרא במטמון קצר, לא בכל בקשה.** המטען הראשי נקרא עשרות
   פעמים בשעה לכל משתמש; קריאת מסמך לכל אחת מהן היתה נוגסת במכסת
   50,000 הקריאות היומיות בלי שום תועלת — הדגל משתנה פעם בחודש.
   ⚠️ **60 שניות, ולא יותר.** זהו גם זמן ההיענות של מתג החירום:
      אחרי `flagSet` לוקח עד דקה עד שכל השרתים מכבים. */
var TX_FS_FLAG_TTL_SEC = 60;
...
function txJobsUseFirestore_() {
  ...
  if (c) {
    try {
      var hit = c.get('cba_flag_txfs');
      if (hit === '1') return true;
      if (hit === '0') return false;
    } catch (e) { /* ערך פגום — נקרא מחדש */ }
  }
  var on = false;
  try { on = (fsGet_(FS_FLAGS_DOC) || {}).budgetTxFromFirestore === true; }
  catch (e) { on = false; }   /* ספק — הגיליון */
  if (c) { try { c.put('cba_flag_txfs', on ? '1' : '0', TX_FS_FLAG_TTL_SEC); } catch (e) {} }
  return on;
}
  ...
  try {
    ...
    try {
      sendResidentTemplate_(ss, 'GARDEN_REPORT_RECEIVED',
        ...
        { 'שם': perm.firstName || name, 'מזהה': repId, 'כותרת': reportTitle, 'קטגוריה': cat, 'מיקום': place });
      notifyAdmins_(ss, PERM_GARDEN, 'ADMIN_NEW_GARDEN_REPORT',
        { 'שם': name, 'מזהה': repId, 'כותרת': reportTitle, 'קטגוריה': cat, 'מיקום': place });
...
function gardenPlanDoc_(def) {
  return {
    ...
    /* ⚠️ **`order` הוא רמז לסדר תצוגה בלבד — לעולם לא מזהה.**
       זה בדיוק הערך שסירבתי לשמור כ-`row`, וההבחנה אינה משחק מילים:
       כמזהה הוא מסוכן (הוספת שורה מעליו תגרום לעדכן את הפריט הלא נכון);
       כרמז לסדר הוא בדיוק מה שצריך — הוא מבטיח שהמסך ייראה **זהה**
       במסלול Firestore ובמסלול הנפילה-לאחור. הוא עלול להתיישן אחרי
       הוספת שורה באמצע, עד הסנכרון המלא הבא — והנזק הוא קוסמטי בלבד. */
    order:       def.row || 0,
    schema:      1,
    updatedAt:   new Date()
  };
 ...
 * ========================================================================== */
/* ⚠️ **הרשימה הזאת חייבת להיות זהה ל-`gtNotifyOk` בכללי האבטחה.**
   הכלל קובע איזה שם תבנית הדפדפן רשאי לכתוב; הרשימה כאן קובעת
   על איזה שם השרת באמת שולח. שם שקיים רק בצד אחד = דגל שיורד
   בשקט בלי מייל, או כתיבה שנדחית — ובשני המקרים אף אחד לא מבחין. */
 ...
 *     בגיליון לא אמור לרוקן שורה במייל. */
function gardenWeekLabel_(w) {
  var m = /^(\d{4})-(\d{2})-(\d{2})$/.exec(String(w || '').trim());
...
function gardenAiSchedule_(ss, body) {
  ...
  var prefs = {
    ...
    /* ⚖️ GW-25.9:C9-spread — ברירת מחדל: לפזר על כל הימים שנבחרו. (יועד, 25.9:
       "בחרתי ראשון–חמישי והוא סידר רק ראשון ושני" — הכלל הקודם "למלא את
       תחילת השבוע קודם" דחס את הכול לימים הראשונים.) false = הכי מוקדם. */
  ...
  var rules = [
    ...
    prefs.spread
      ? '- ⚖️ לפזר את העבודה באופן מאוזן על **כל** הימים שב-days — הגנן בחר אותם בכוונה. סך הדקות המשובצות בכל יום (ולכל עובד) קרוב ככל האפשר לממוצע; אסור למלא יום או יומיים ולהשאיר ימים אחרים ריקים כשיש משימות. העדפות "מוקדם בשבוע" שלמעלה חלות רק על המשימות שהן מזכירות. בתוך יום — להתחיל מתחילת חלון העבודה, בלי חורים מיותרים.'
...
function handleGardenStats_(p) {
  try {
    ...
    /* 🔴🔴 **ממצא 20, סעיף (2) — משימה סגורה שקיבלה משוב שלילי אינה גמורה**
       (2026-09-22).
       ------------------------------------------------------------------
       התלונה בדוח הייתה ש"הסטטוס נשאר בוצע, ולכן הכרטיס והמדדים מספרים
       סיפור אחר ממה שקרה בשטח", והמסקנה שהוצעה שם הייתה **לפתוח את
       המשימה מחדש אוטומטית**.
       🔑 **לא עשינו את זה, ובכוונה.** §18.3 באפיון קובע שמשוב שלילי
          מרים דגל וההחלטה נשארת אנושית — ומטעם טוב: פתיחה אוטומטית
          נותנת לתושב לבטל החלטה של הצוות בלחיצה. מה שהיה שבור הוא לא
          הסטטוס אלא **המדידה**: משימה שמחכה לבדיקה חוזרת נספרה
          כ"נסגרה", ולא נספרה כ"דורשת תשומת לב" — כלומר היא נעלמה
          משני האריחים בבת אחת, וזה בדיוק מה שהופך מסך נתונים למסך
          שמרגיע במקום להתריע.
       מהיום היא נספרת איפה שהיא באמת נמצאת: בתשומת הלב, לא בסגורות.
       ⚠️ הדגל יורד ברגע שהמנהל מחליט (פתיחה מחדש או סגירה סופית),
          ואז המשימה חוזרת לספירה הרגילה מעצמה. אין כאן מצב נוסף
          לתחזק — רק שאילתה שמסכימה עם הדגל שכבר קיים. */
    function gardenUnsettled_(t) {
      return !!t.closure && t.flag === 'דורש בדיקה חוזרת';
    }

    tasks.forEach(function (t) {
      if (gardenUnsettled_(t)) { now.attention++; return; }
      if (t.closure) {
    ...
    tasks.forEach(function (t) {
      ...
      if (!a) return;
      /* אותה הבחנה בדיוק כמו ב"מצב עכשיו" למעלה — אזור שיש בו תקלה
         שממתינה לבדיקה חוזרת אינו אזור שסגר אותה. */
      if (gardenUnsettled_(t)) { a.attention++; return; }
 ...
 *  `note` היא הערת הביצוע (חובה בדיווח תושב) והיא נכנסת ל-{{מה נעשה}}. */
function gardenNotifyCompleted_(ss, taskId, note) {
  try {
    var reps = gardenReportsForTask_(ss, taskId);
    for (var i = 0; i < reps.length; i++) {
      var rep = reps[i];
      ...
      sendResidentTemplate_(ss, 'GARDEN_COMPLETED', emailsForFamilyId_(ss, rep.familyId), {
        ...
        'איחוד': rep.mergedInto
          ? 'הדיווח שלך אוחד עם דיווח מס\' ' + rep.mergedInto + '.\n\n' : '',
 ...
 *
 *  ⚠️ **עודכן 2026-09-09 (החלטת יועד): גם דיווח תושב נסגר בסימון.**
 *  ההבחנה עברה מ"מי מחכה" ל**סוג ההחלטה**: המנהל מאשר *סירובים ומחלוקות,
 *  לא ביצועים*. מכתב שאומר "לא נטפל בזה" הוא החלטה וצריך אדם; מכתב שאומר
 *  "טופל" הוא דיווח — ואם הוא שגוי, התושב מתקן אותו בכפתור "לא הושלם".
 *  זה הופך את התושב עצמו למנגנון הבקרה, במקום להעמיס תור על המנהל.
 *    · דיווח תושב — הסימון סוגר, **בתנאי שנכתבה הערת ביצוע** שנכנסת למייל.
 *    · שגרה ויזום — הסימון סוגר. אין מי שמחכה.
 *    · סגירה בסיבה, "לא ניתן לביצוע", "לא הושלם" של תושב — אצל המנהל.
 *
...
function gardenCloseRow_(ss, sh, row, c, cur, closure, who, why) {
  ...
  gardenLog_(ss, cur.id, 'סגירה', 'סגירה', cur.closure, closure, who,
             String(why || '').trim());
  /* ⚠️ 2026-09-09 — הערת הביצוע עוברת למייל. היא הפכה לחובה בדיווח תושב
     בדיוק בשביל זה: המכתב אומר **מה נעשה**, לא רק "הושלם". */
  if (closure === 'בוצע') {
    gardenNotifyCompleted_(ss, cur.id, String(why || cur.note || '').trim());
    return;
  }
...
/* ---------- פעולות על משימה (doPost) ----------
   ...
   לביצוע בשטח: סימון, הערה, דחייה, חסימה.
   ⚠️ 2026-09-09 — **הסימון סוגר עכשיו גם דיווח תושב** (ר' הבלוק מעל
   gardenCloseRow_). הפעולות שנשארו למנהל בלבד הן `approve`/`close`/`return`/
   `clearflag`, והן נבדקות ב-perm.isExternal בהמשך הפונקציה. */
function gardenTaskAction_(ss, body) {
  ...
  try {
    ...
    /* משימה שנסגרה בסימון של הצוות נשארת פתוחה לפתיחה מחדש בתוך חלון
       הערעור — זה מה שהחליף את האישור מראש.
       ⚠️ 2026-09-09 — **גם דיווח תושב נכנס לכאן עכשיו.** קודם הוא הוחרג
       בנימוק "הוא ממילא עובר אישור"; משבוטל האישור, ההחרגה הייתה יוצרת מצב
       בלי בקרה בכלל: לא אישור מראש ולא ערעור בדיעבד. */
    /* ⚠️ 2026-09-17, גל 2 · ממצא 22 — **מנהל הגינון פותח מחדש כל סגירה,
       בלי הגבלת זמן.** עד היום פתיחה מחדש הותרה רק על סגירה 'בוצע' ורק
       בתוך חלון הערעור, ולכן סגירה בסיבה שגויה ("הועבר לבינוי" במקום
       "בוטל"), וכל סגירה של דיווח תושב, היו **בלתי הפיכות** — גם למנהל-על.
       הגנן החיצוני נשאר עם חלון הערעור על הסגירה העצמית שלו בלבד: זה מה
       שהחליף את האישור מראש, וזה עדיין הגבול הנכון עבורו.
       ⚠️ מחיקה אינה עוברת כאן כלל — היא פעולה נפרדת (gardenTaskDelete_)
          שמעולם לא בדקה סגירה. מה שחסם אותה בפועל היה הלקוח. */
    if (cur.closure) {
      var isMgr = !perm.isExternal;
      var reopenAct = (act === 'undo' || act === 'return');
      var reopenable = reopenAct && (isMgr ||
                       (cur.closure === 'בוצע' && gardenWithinDispute_(cur)));
    ...
    if (act === 'done') {
      /* ⚠️ 2026-09-09, החלטת יועד — **הסימון סוגר, גם בדיווח תושב.**
         הכלל החדש: המנהל מאשר סירובים ומחלוקות, לא ביצועים. הודעה שאומרת
         "טופל" היא דיווח ולא החלטה, ואם היא שגויה — התושב עצמו מתקן אותה
         בכפתור "לא הושלם" שכבר קיים.
         שלוש רשתות הביטחון שמחליפות את האישור:
           1. **הערת ביצוע חובה** בדיווח תושב — משפט אחד על מה נעשה, שנכנס
              למייל. בלעדיו אין סגירה. זה גם מה שהופך את המייל למשהו שאפשר
              לערער עליו.
           2. חלון הערעור של הגנן (undo) — ר' reopenable מעל.
           3. "לא הושלם" של התושב, שמרים 'דורש בדיקה חוזרת' אל המנהל. */
      var doneNote = String(body.note || '').trim().substring(0, 500);
      if (cur.kind === GARDEN_KIND_REPORT && !doneNote) {
        return { ok: false,
                 error: 'צריך לכתוב מה נעשה — המשפט הזה נשלח לתושב שדיווח.' };
      }
      if (doneNote) {
        gardenSet_(sh, row, c, 'הערת ביצוע', doneNote);
        gardenLog_(ss, id, 'הערה', 'הערת ביצוע', cur.note, doneNote, who, '');
        cur.note = doneNote;
      }
      gardenLog_(ss, id, 'ביצוע', 'דגל', cur.flag, 'בוצע', who, '');
      gardenCloseRow_(ss, sh, row, c, cur, 'בוצע', who, doneNote);
      ...
      if (cur.closure) {
        ...
        gardenLog_(ss, id, 'ביטול ביצוע', 'סגירה', cur.closure, '', who,
