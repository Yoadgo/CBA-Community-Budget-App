  ...
  function gardenPlanRead(cb) {
    fsFirstRead("gardenPlan", GARDEN_PLAN_FROM_FIRESTORE, function (done) {
      ...
      CBA.fb.readCollection("gardenPlan", function (err, rows) {
        ...
        /* 🔴🔴 **השומר "אוסף ריק = כשל" הוסר — 22.9.2026.**
           ההנחה שמאחוריו הייתה נכונה כל עוד Firestore היה **מראה**:
           אוסף ריק פירושו שהסנכרון מהגיליון לא רץ, ומסך ריק היה שקר.
           מרגע ש-`gardenWritesFromBrowser` דלוק אין מי שימלא את האוסף
           מבחוץ — Firestore **הוא** המקור, וריק הוא האמת.
           🔑 **ומה שהפך את זה מטעות-תיעוד לתקלה חוסמת:** המסך שבו
              מזינים את שורת התוכנית הראשונה הוא אותו מסך שסירב
              להיטען כשהתוכנית ריקה. מעגל סגור — אי אפשר היה להתחיל.
           ⚠️ כשל אמיתי (הרשאה, רשת, כללים) עדיין נתפס ב-`err` למעלה
              ועדיין מציג "לא הצלחתי לטעון" עם "נסה שוב". מה שהשתנה
              הוא רק שריק אינו נחשב כשל.
           ⚠️ `rows || []` ולא `rows` — `maybeDone` בודק `!defs`,
              ומערך ריק אמנם אמיתי אבל `null` היה תולה את הקריאה
              לנצח בלי שגיאה. */
        defs = rows || [];
        maybeDone();
      });
  ...
  function gardenTasksRead(opts, cb) {
    ...
    fsFirstRead("gardenTasks", GARDEN_TASKS_FROM_FIRESTORE, function (done) {
      CBA.fb.readCollection("gardenTasks", function (err, rows) {
        ...
        /* 🔴🔴 **אותו שומר, אותה הסרה — 22.9.2026.** ההערה שהייתה כאן
           אמרה "לשיכון תמיד יש משימות גינון; ריק פירושו שמשהו השתבש".
