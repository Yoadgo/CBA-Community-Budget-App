...
(function () {
  ...
  /* ==== חוצץ חודשי (2026-09-07) ====
     ...
     גרסה קודמת הייתה בלוק סיכום יחיד עם אקורדיון — סטייה מההנחיה, הוחלפה.

     ⚠️ 8.9.26: הבלוק הזה כבר נמחק פעם אחת ע"י סשן מקביל שכתב חזרה גרסה ישנה
     של הקובץ, בזמן ש-resident.css נשאר עם `.rq-mo`. התוצאה: JS שפולט `.rq-sum`
     מול CSS שמכיר רק `.rq-mo` — **מסך שבור בייצור**. אם `.rq-mo` קיים ב-CSS,
     הפונקציות כאן חייבות להתקיים. ר' cba-resident-refund-summary-2026-09-07.
  ...
  function monthDividerHTML(monthIso, items) {
    ...
    // הניסוח נגזר מהסטטוס ולא מהתאריך: חודש שעבר אך טרם שולם לא יטען שהכסף נכנס.
    var when = nSettled
